/**
 * Ollama API üzerinden DeepSeek R1 modelini çağıran modül.
 * Local Ollama servisine HTTP isteği gönderir ve makale içeriği üretir.
 */
...
/**
 * Ollama API'sine istek gönderir ve makale içeriği üretir.
 * @param {string} systemPrompt - Sistem prompt'u (rol, stil, format vb.)
 * @param {string} userPrompt - Kullanıcı prompt'u (makale iskeleti ve konu)
 * @returns {Promise<string>} Üretilen makale içeriği
 */
export async function callOllama(systemPrompt, userPrompt) {
  ...
  try {
    ...
    console.log(`[callOllama] Makale içeriği başarıyla üretildi (${content.length} karakter)`);
    return content;
  } catch (error) {
    // Network hataları
